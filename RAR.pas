...
type
  TOnRARErrorNotifyEvent = procedure(Sender: TObject; const ErrorCode: integer;
    const Operation: TRAROperation) of object;
  TOnRARListFile = procedure(Sender: TObject; const FileInformation: TRARFileItem) of Object;
  TOnRARPasswordRequired = procedure(Sender: TObject; const HeaderPassword: boolean;
    const FileName: AnsiString; out NewPassword: AnsiString; out Cancel: boolean) of object;
  TOnRARNextVolumeRequired = procedure(Sender: TObject; const requiredFileName: AnsiString;
    out newFileName: AnsiString; out Cancel: boolean) of Object;
  TOnRARProcess = procedure(Sender: TObject; const FileName: WideString;
    const ArchiveBytesTotal, ArchiveBytesDone, FileBytesTotal, FileBytesDone: cardinal)
    of Object;
  TOnRARReplace = procedure(Sender: TObject; const ExistingData, NewData: TRARReplaceData;
    out Action: TRARReplace) of object;
...
type
  ...
  public
    ...
    // if true, mv's will be read until last part of the file
    ...
    property OnPasswordRequired: TOnRARPasswordRequired read fOnPasswordRequired
      write fOnPasswordRequired;
    property OnNextVolumeRequired: TOnRARNextVolumeRequired read fOnNextVolumeRequired
      write fOnNextVolumeRequired;
...
begin
  ...
  case msg of
    UCM_CHANGEVOLUME:
      begin
        ...
        case P2 of
          RAR_VOL_ASK:
            begin
              ...
              begin
                ...
                // todo: handle error if P1 has not enough space for FileName
          ...
          RAR_VOL_NOTIFY:
            begin // occurs when next volume required and next part was found
              Result := 0; // continue
            end;
        end;
    ...
    UCM_NEEDPASSWORD:
      begin
        if not fArchiveInformation.fOpened then
        begin
        ...
        if assigned(fOnPasswordRequired) then
          fOnPasswordRequired(Self, not fArchiveInformation.fOpened, PasswordFile,
            Password, Cancel);
    ...
    UCM_PROCESSDATA:
      begin
        ...
        if assigned(fOnProcess) then
          fOnProcess(Self, fProgressInfo.FileName, fArchiveInformation.UnCompressedSize,
            fProgressInfo.TotalSize, fProgressInfo.FileBytesTotal,
            fProgressInfo.FileBytesDone);
        if fAbort then
          Result := -1
        else
          Result := 0;
...
begin
  ...
  begin // handle incorrect=failed to load dll
    Error(ERAR_DLL_LOAD_ERROR, roInitArchive);
    Result := False;
    Exit;
  end;
  ...
  case ArchiveData.CmtState of // read archive comment
    ...
    ERAR_NO_MEMORY:
      Error(ERAR_NO_MEMORY, roInitArchive);
    ERAR_BAD_DATA:
      Error(ERAR_BAD_DATA, roInitArchive);
    ERAR_UNKNOWN_FORMAT:
      Error(ERAR_UNKNOWN_FORMAT, roInitArchive);
    ERAR_SMALL_BUF:
      Error(ERAR_SMALL_BUF, roInitArchive);
  end;
  if (ArchiveData.CmtState <> ERAR_NO_COMMENTS) and
    (ArchiveData.CmtState <> ERAR_COMMENTS_EXISTS) then
...
var
  ...
  OS: String;
begin
  if (fReadMVToEnd) and (not((hdrData.Flags and $00000001) = $00000001)) and
  // first part of the file
    (((hdrData.Flags and $00000002) = $00000002)) then
    fPackedSizeMVVolume := hdrData.PackSize;
  if (fReadMVToEnd) and (((hdrData.Flags and $00000001) = $00000001)) and
  ...
  if fArchiveInformation.fArchiverMajorVersion * 10 + fArchiveInformation.fArchiverMinorVersion
    < hdrData.UnpVer then
  begin
    fArchiveInformation.fArchiverMinorVersion := hdrData.UnpVer mod 10;
    fArchiveInformation.fArchiverMajorVersion :=
      (hdrData.UnpVer - fArchiveInformation.fArchiverMinorVersion) div 10;
  end;
  ...
  if (not((hdrData.Flags and $00000070) = $00000070)) and
    (hdrData.FileAttr <> faDirectory) then
  begin // not a directory
    ...
    case (hdrData.Flags shl 24 shr 29) of
      0:
        fArchiveInformation.fDictionarySize := 65536;
      1:
        fArchiveInformation.fDictionarySize := 131072;
      2:
        fArchiveInformation.fDictionarySize := 262144;
      3:
        fArchiveInformation.fDictionarySize := 524288;
      4:
        fArchiveInformation.fDictionarySize := 1048576;
      5:
        fArchiveInformation.fDictionarySize := 2097152;
      6:
        fArchiveInformation.fDictionarySize := 4194304;
    end;
  end;
  fArchiveInformation.fCompressedSize := fArchiveInformation.fCompressedSize +
    hdrData.PackSize;
  fArchiveInformation.fUnCompressedSize := fArchiveInformation.fUnCompressedSize +
    hdrData.UnpSize;
  if ((hdrData.Flags and $00000001) = $00000001) or ((hdrData.Flags and $00000002) = $00000002)
  then // file continued in last or next part
...
begin
  ...
  try
    ...
    while (ReadFileHeaderResult = RAR_SUCCESS) and Result do
    begin
...
function extractFile(FileName: String; Files: TStrings): boolean;
// returns if the actual file should be extracted or not
var
  i: integer;
begin
  if Files = NIL then
...
begin
  ...
  try
    ...
    begin
      ...
      if extractFile(StrPas(hdrData.FileName), Files) then
      begin // todo: UniCode FileName
        ...
        case ReplaceResult of
          ...
          rrOverwrite:
            if RestoreFolder then
              ReadFileHeaderResult := RARProcessFile(ArchiveHandle, RAR_EXTRACT,
               PAnsiChar(Path), NIL)
            else if (not((hdrData.Flags and $00000070) = $00000070)) and
              (hdrData.FileAttr <> faDirectory) then
              ReadFileHeaderResult := RARProcessFile(ArchiveHandle, RAR_EXTRACT, Nil,
                PAnsiChar(ExistentFile.FileName));
          rrSkip:
            begin
              ReadFileHeaderResult := RARProcessFile(ArchiveHandle, RAR_SKIP,
                PAnsiChar(Path), NIL);
...
{$WARN COMBINING_SIGNED_UNSIGNED ON}
        ...
        ReadFileHeaderResult := RARProcessFile(ArchiveHandle, RAR_SKIP, NIL, NIL);
      // select next file without extracting
...
begin
  ...
  begin
    ...
    if (@RAROpenArchive = nil) or (@RAROpenArchiveEx = nil) or (@RARCloseArchive = nil) or
      (@RARReadHeader = nil) or (@RARReadHeaderEx = nil) or (@RARProcessFile = nil) or
      (@RARSetCallback = nil) or (@RARSetChangeVolProc = nil) or (@RARSetProcessDataProc = nil)
      or (@RARSetPassword = nil) or (@RARGetDllVersion = nil) then
    begin
      RARDLLInstance := 0;
      UnloadDLL;
    end;
...
begin
  ...
  // (ErrorCode=ERAR_END_ARCHIVE) or
    (ErrorCode = ERAR_NO_MEMORY) or (ErrorCode = ERAR_BAD_DATA) or
    (ErrorCode = ERAR_UNKNOWN_FORMAT) or (ErrorCode = ERAR_EOPEN) or (ErrorCode = ERAR_ECREATE)
    or (ErrorCode = ERAR_ECLOSE) or (ErrorCode = ERAR_EREAD) or (ErrorCode = ERAR_EWRITE) or
    (ErrorCode = ERAR_SMALL_BUF) or (ErrorCode = ERAR_UNKNOWN) then
    fAbort := True;
